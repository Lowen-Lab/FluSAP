...
verbose = False
min_cov = 50 #only for linked iSNV analysis
...
if skip_linked_SNV_counting == False:
	lnk_dir = map_dir+"lnkSNV/"
	if not os.path.exists(lnk_dir):
		os.makedirs(lnk_dir)
	if not os.path.exists(lnk_dir+ref_set.split("-")[0].split(".f")[0]):
		os.makedirs(lnk_dir+ref_set.split("-")[0].split(".f")[0])
	...
	if all_valid_operators == True:
		for num in range(0,len(read_key)):
			...
			if ref_loc != '':
				...
				# if qual_score >= 30 and num >= 30 and R_end_dist>=30 and mapping_qual>=30:
...
def main_pipeline(sample_ref_tup,ref_set_list,input_read_dir,ref_dir,trim_dir,map_dir,qual_dir,sam_dir,bam_dir,temp_dir,sub_dir,command_prefix,dedupe_reads,min_cov,min_proportion):
	max_indel_count = 0
				...
				if len(sam_line) > 0:
					if sam_line[0] != "@":
						...
						if read_seq !="*":# and len(read_seq) >= min_read_length:#map_qual >= min_mapping_qual and 
							...
							# read_base_info_list,CIGAR_operator_count_dict = SAM_parse(map_site,cigar,read_seq_qual,read_seq,map_qual)
							if len(read_base_dict) > 0: #per base in read: (read_nt,qual_score,mapping_qual,read_length,num,R_end_dist,nt_prop_location)
								...
								if indel_count <= max_indel_count and mismatch_count <= max_mismatch_count:
									...
									try:
										...
										# prev_miss_count = [readID+"-"+read_seq[0:6]][1]
										# prev_indel_count = read_map_info_dict[readID+"-"+read_seq[0:6]][2]
					...
					for num2 in range(num1,len(read_subIDs)):
						if num1 != num2:
							...
							try:
								...
								# print(pair+"\t"+str(segment_isnv_link_count_dict[pair]))
							except:
								segment_isnv_link_count_dict[pair] = 1
	if verbose == True:
	...
	if skip_qual == False:
		site_loc_outfilename = temp_dir+sampleID+"."+primary_refID+".read_loc_hist.txt"
		read_len_outfilename = temp_dir+sampleID+"."+primary_refID+".read_len_hist.txt"
		prop_loc_outfilename = temp_dir+sampleID+"."+primary_refID+".read_prop_loc_hist.txt"
		indel_outfilename = temp_dir+sampleID+"."+primary_refID+".indel_count_hist.txt"
		mismatch_outfilename = temp_dir+sampleID+"."+primary_refID+".mismatch_count_hist.txt"
		map_qual_outfilename = temp_dir+sampleID+"."+primary_refID+".base_map_qual.txt"

		out_site_loc_outfilename = qual_dir+primary_refID+"/"+sampleID+".read_loc_hist.txt"
		out_read_len_outfilename = qual_dir+primary_refID+"/"+sampleID+".read_len_hist.txt"
		out_prop_loc_outfilename = qual_dir+primary_refID+"/"+sampleID+".read_prop_loc_hist.txt"
		out_indel_outfilename = qual_dir+primary_refID+"/"+sampleID+".indel_count_hist.txt"
		out_mismatch_outfilename = qual_dir+primary_refID+"/"+sampleID+".mismatch_count_hist.txt"
		out_map_qual_outfilename = qual_dir+primary_refID+"/"+sampleID+".base_map_qual.txt"
	...
	temp_substitutions_filename = temp_dir+sampleID+"."+primary_refID+".substitions.txt"
	out_substitutions_filename = sub_dir+primary_refID+"/"+sampleID+"."+primary_refID+".substitions.txt"
		...
		for loc in range(0,max_loc):
			...
			for i in range(0,len(bases)):
				...
				if prop_base >= min_proportion:
					...
					z = -1*step_size
					while z < 1.0:#for z in np.arange(0.0, 1.0, step_size):
						z += step_size
						counter = 0
	...
	if skip_linked_SNV_counting == False:
		# print("Processing linked iSNVs")
		link_iSNV_outlines = ''
		lnk_temp_filename = temp_dir+sampleID+"."+refID+".lnSNVs.txt"
		lnk_out_filename = lnk_dir+primary_refID+"/"+sampleID+"."+refID+".lnSNVs.txt"
		...
		for pair in segment_isnv_link_count_dict:
			...
			nt2 = isnv2[-1]
			try:
				group_1 = cdSNP_dict[segment][loc1][nt1].replace("H1N1","group1").replace("H3N2","group2")
			except:
				group_1 = 'na'
			try:
				group_2 = cdSNP_dict[segment][loc2][nt2].replace("H1N1","group1").replace("H3N2","group2")
			except:
				group_2 = 'na'
			try:
				cov1 = count_dict[loc1]
				cov2 = count_dict[loc2]
			except:
				cov1 = 0
				cov2 = 0
			if cov1 >= min_cov and cov2 >= min_cov:
				try:
					nt_count1 = allele_count_dict[loc1][nt1]
				except:
					nt_count1 = 0
				sub_prop1 = round((nt_count1/cov1),4)

				try:
					nt_count2 = allele_count_dict[loc2][nt2]
				except:
					nt_count2 = 0
				sub_prop2 = round((nt_count2/cov2),4)
				if cov1 >= min_cov and cov2 >= min_cov:
					if sub_prop1 >= min_proportion and sub_prop2 >= min_proportion:
						if (sub_prop1 <= 0.5 and sub_prop2 <= (1.0-min_proportion)) or (sub_prop2 <= 0.5 and sub_prop1 <= (1.0-min_proportion)):
							try:
								avg_qual_1 = round((sub_info_dict[loc1][nt1][0]/nt_count1),1)
								avg_map_qual_1 = round((sub_info_dict[loc1][nt1][1]/nt_count1),1)
								avg_read_len_1 = round((sub_info_dict[loc1][nt1][2]/nt_count1),1)
								min_avg_loc_1 = round((sub_info_dict[loc1][nt1][3]/nt_count1),1)
								avg_mismatch_count_1 = round((sub_info_dict[loc1][nt1][6]/nt_count1),1)
								avg_indel_count_1 = round((sub_info_dict[loc1][nt1][7]/nt_count1),1)
							except:
								avg_qual_1 = 'na'
								avg_map_qual_1 = 'na'
								avg_read_len_1 = 'na'
								min_avg_loc_1 = 'na'
								avg_mismatch_count_1 = 'na'
								avg_indel_count_1 = 'na'
							# if avg_qual_1 >= 35 and avg_map_qual_1 >=30 and
							try:
								avg_qual_2 = round((sub_info_dict[loc2][nt2][0]/nt_count2),1)
								avg_map_qual_2 = round((sub_info_dict[loc2][nt2][1]/nt_count2),1)
								avg_read_len_2 = round((sub_info_dict[loc2][nt2][2]/nt_count2),1)
								min_avg_loc_2 = round((sub_info_dict[loc2][nt2][3]/nt_count2),1)
								avg_mismatch_count_2 = round((sub_info_dict[loc2][nt2][6]/nt_count2),1)
								avg_indel_count_2 = round((sub_info_dict[loc2][nt2][7]/nt_count2),1)
							except:
								avg_qual_2 = 'na'
								avg_map_qual_2 = 'na'
								avg_read_len_2 = 'na'
								min_avg_loc_2 = 'na'
								avg_mismatch_count_2 = 'na'
								avg_indel_count_2 = 'na'
							link_iSNV_outlines += sampleID +"\t"+ segment +"\t"+ pair.replace('_','\t') +"\t"+ str(link_count) +"\t"+ group_1 +"\t"+ group_2 +"\t"
							link_iSNV_outlines += str(cov1) +"\t"+ str(sub_prop1) +"\t"+ str(avg_qual_1) +"\t"+ str(avg_map_qual_1) +"\t"+ str(avg_read_len_1) +"\t"+ str(min_avg_loc_1) +"\t"+ str(avg_mismatch_count_1) +"\t"+ str(avg_indel_count_1) +"\t"
							link_iSNV_outlines += str(cov2) +"\t"+ str(sub_prop2) +"\t"+ str(avg_qual_2) +"\t"+ str(avg_map_qual_2) +"\t"+ str(avg_read_len_2) +"\t"+ str(min_avg_loc_2) +"\t"+ str(avg_mismatch_count_2) +"\t"+ str(avg_indel_count_2) +"\n"
...
if parallel_process == False:
	for sample_ref_tup in sample_ref_pair_list:
		# sampleID = sample_ref_tup[0]
		# ref_set = sample_ref_tup[1]
		main_pipeline(sample_ref_tup,ref_set_list,input_read_dir,ref_dir,trim_dir,map_dir,qual_dir,sam_dir,bam_dir,temp_dir,sub_dir,command_prefix,dedupe_reads,min_cov,min_proportion)
elif parallel_process == True:
	print(num_cores)
	processed_list = Parallel(n_jobs=num_cores)(delayed(main_pipeline)(sample_ref_tup,ref_set_list,input_read_dir,ref_dir,trim_dir,map_dir,qual_dir,sam_dir,bam_dir,temp_dir,sub_dir,command_prefix,dedupe_reads,min_cov,min_proportion) for sample_ref_tup in sample_ref_pair_list)
